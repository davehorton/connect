...
class DrachtioAgent extends Emitter {
  ...
  _normalizeParams(socket, uri, options, callback) {
    ...
    if (options._socket) {
      debugSocket('_normalizeParams: using socket provided in options._socket');
  ...
  _makeRequest(params) {
    debugSocket(`_makeRequest: there are ${this.mapServer.length} entries in mapServer`);
  ...
  disconnect(socket) {
    ...
    if (socket) {
      ...
      debugSocket(`disconnect: after delete there are ${this.mapServer.length} entries in mapServer`);
    }
  }
  close() {
    this.wp.close() ;
  ...
  _getDefaultSocket() {
    debugSocket(`_getDefaultSocket: there are ${this.mapServer.length} entries in mapServer`);
  ...
  _initServer(socket) {
    ...
    debugSocket(`_initServer: there are ${this.mapServer.length} entries in mapServer after adding new socket`);
