...
class DrachtioAgent extends Emitter {
  ...
  get idle() {
    ...
    const pendingSipCount = this.pendingSipRequests.size ;
    const pendingAckOrPrack = this.pendingAckOrPrack.size ;
    if (pendingCount > 0) {
    ...
    if (pendingSipCount > 0) {
      ...
      for (const key of this.pendingSipRequests.keys()) {
        debug(key);
      }
    }
    if (pendingAckOrPrack > 0) {
      debug(`count of pending ack/prack: ${pendingAckOrPrack}`) ;
      for (const key of this.pendingAckOrPrack.keys()) {
        debug(key);
      }
    }
  ...
  request(request_uri, options, callback) {
    const params = this._normalizeParams(request_uri, options, callback) ;
    return this._makeRequest(params) ;
  }

  sendResponse(res, opts, callback, fnAck) {
    debug(`agent#sendResponse: ${JSON.stringify(res.msg)}`);
    const msgId = this.sendMessage(res.msg, Object.assign({stackTxnId: res.req.stackTxnId}, opts)) ;
    if (callback || fnAck) {
    ...
    if (res.statusCode >= 200) {
      ...
      if (res.req.method === 'INVITE') {
        ...
        debug(`Agent#sendResponse: deleted pending invite for call-id ${callId}, ` +
          `there are now ${this.pendingNetworkInvites.size} pending invites`);
      }
    }
  }
  ...
  sendAck(method, dialogId, req, res, opts, callback) {
    ...
    const msgId = this.sendMessage(m, opts) ;
    if (callback) {
      this.pendingRequests.set(msgId, (token, msg, meta) => {
        if ('OK' !== token[0]) {
  ...
  _onConnect() {
    const msgId = this.wp.send(`authenticate|${this.secret}|${(this.label || 'default')}`) ;
    this.pendingRequests.set(msgId, (response) => {
      ...
      if (this.authenticated) {
        ...
        const self = this ;
        setTimeout(() => {
          self.emit('connect', null, response[1]) ;
        }, 100) ;
          ...
          if ('network' === source && sipMsg.type === 'request') {
            ...
            if ('CANCEL' === sipMsg.method && this.pendingNetworkInvites.has(callId)) {
              ...
              debug(`Agent#handle - emitted cancel event for INVITE with call-id ${callId}` +
                `, remaining count of invites in progress: ${this.pendingNetworkInvites.size}`);
              return ;
            }
            ...
            if ('INVITE' === req.method) {
              ...
              debug(`Agent#handle: tracking an incoming invite with call-id ${callId}, ` +
                `currently tracking ${this.pendingNetworkInvites.size} invites in progress`);
            }
