...
var debug = require('debug')('drachtio');
	...
	case 'sip':
		...
		if( token.length > 9 ) {
			...
			if( 'network' === source && sipMsg.type === 'request' ) {
				...
				else if( ('PRACK' === req.method || 'ACK' === req.method) && dialogId in agent.pendingAckOrPrack ) {
					var fnAck = agent.pendingAckOrPrack[dialogId];
					delete agent.pendingAckOrPrack[dialogId];
					fnAck() ;
				}
...
function Agent( cb ){
	...
	this.__defineGetter__('idle', function() {
		...
		var pendingAckOrPrack = Object.keys(this.pendingAckOrPrack).length ;
		if( pendingCount > 0 ) {
		...
		if( pendingAckOrPrack > 0 ) {
			console.log('count of pending prack/ack: %d', pendingAckOrPrack) ;
			console.log(util.inspect(this.pendingAckOrPrack)) ;
		}
...
Agent.prototype.sendResponse = function( res, opts, callback, fnAck ) {
	var self = this ;
	...
	if( callback ) {
		this.pendingRequests[msgId] = function(token, msg, meta) {
			...
			var responseMsg = new SipMessage(msg) ;
			res.meta = meta ;
			callback(null, responseMsg ) ;

			// for reliable provisional responses or does caller want to be notified on receipt of prack / ack ?
			if( fnAck && responseMsg.has('RSeq') || res.status === 200 ) {
				self.pendingAckOrPrack[meta.dialogId] = fnAck ;
			}
		} ;
	}
	if( res.status >= 200 ) { 
...
Agent.prototype.connection_gone = function (why) {
	...
	if (this.retry_timer) {
			debug('Agent#connection_gone: retry is already in progress') ;
	    return;
	}
