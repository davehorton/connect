...
module.exports = class WireProtocol extends Emitter {
  ...
  _onConnectionGone() {
    ...
    let nextDelay = Math.floor(this.reconnectVars.retryDelay * this.reconnectVars.retryBackoff);
    ...
    this.reconnectVars.retryTimer = setTimeout( () => {
        ...
        if (this.reconnectOpts.connectTimeout && this.reconnectVars.retryTotaltime >= this.reconnectOpts.connectTimeout) {
            this.reconnectVars.retryTimer = null;
            console.error(`WireProtocol#connection_gone: Couldn't get drachtio connection after ${this.reconnectVars.retryTotaltime} ms`);
            return;
        }
        this.socket = net.connect({
          port: this.port,
  ...
  send( msg ) {
    ...
    this.logger( '===>' + CRLF + s.length + '#' + s + CRLF) ;
    return msgId ;
  } 
  ...
  parseMessageHeader( msg, hashPosition ) {
    ...
    let start = ++hashPosition, end = start + len ;
