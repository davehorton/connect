...
module.exports = class WireProtocol extends Emitter {
  ...
  parseMessageHeader(msg, hashPosition, obj) {
    ...
    msg = Buffer.byteLength(msg, 'utf8') === (end + 1) ? '' : msg.slice(hashPosition + len) ;
  ...
  _onData(socket, msg) {
    ...
    try {
      while (Buffer.byteLength(msg, 'utf8') > 0) {
        let pos ;
        if (0 === Buffer.byteLength(obj.incomingMsg, 'utf8')) {
          ...
          var remainderSize = obj.incomingMsgLength - Buffer.byteLength(obj.incomingMsg, 'utf8') ;
          obj.incomingMsg += msg.slice(0, remainderSize) ;
          msg = msg.slice(remainderSize) ;
        }

        //if we've got a full message, process it
        if (Buffer.byteLength(obj.incomingMsg, 'utf8') === obj.incomingMsgLength) {
